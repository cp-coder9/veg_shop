...
export default function PublicProductList() {
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen bg-warm-gray-50 flex items-center justify-center">
                <div className="w-12 h-12 border-4 border-organic-green-300 border-t-organic-green-600 rounded-full animate-spin"></div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-warm-gray-50">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <Leaf className="text-organic-green-500 w-8 h-8" />
                            <h1 className="font-display font-bold text-2xl text-organic-green-800 tracking-tight">Our Fresh List</h1>
                        </div>
                        <div className="relative w-64">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-4 py-2 bg-warm-gray-100 border-transparent rounded-full text-sm focus:bg-white focus:ring-2 focus:ring-organic-green-400 transition-all"
                            />
                        </div>
                    </div>
                </div>
            ...
            {/* Hero */}
            <section className="bg-organic-green-800 text-white py-16 overflow-hidden relative">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <div className="max-w-2xl">
                        ...
                        <p className="text-organic-green-50 text-lg mb-8 max-w-lg">
                            Check our available harvest this week. Freshly picked, organically grown, and hand-delivered.
                        </p>
                        <a
                            href="/login"
                            className="inline-block bg-white text-organic-green-800 font-bold px-8 py-4 rounded-xl hover:bg-organic-green-300 transition-colors shadow-xl"
                        >
                            Sign up to Order
                        </a>
                    </div>
                ...
                <div className="absolute -right-20 -bottom-20 w-96 h-96 bg-organic-green-700 rounded-full blur-3xl opacity-50"></div>
            </section>
                    ...
                    Object.entries(groupedProducts).map(([category, items]) => (
                        <div key={category} className="mb-16 last:mb-0">
                            <h3 className="text-xl font-display font-bold text-warm-gray-900 mb-8 flex items-center gap-3">
                                <span className="w-8 h-0.5 bg-organic-green-400"></span>
                                    ...
                                    <div key={product.id} className="bg-white rounded-2xl border border-warm-gray-200 overflow-hidden group hover:shadow-2xl transition-all duration-300">
                                        <div className="relative aspect-square">
                                            {product.imageUrl ? (
                                                ...
                                                <div className="w-full h-full bg-organic-green-400 flex items-center justify-center">
                                                    <Leaf className="w-12 h-12 text-organic-green-300" />
                                                </div>
                                            )}
                                        ...
                                        <div className="p-6">
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <span className="text-2xl font-black text-organic-green-800">R{(Number(product.price) / 100).toFixed(2)}</span>
                                                    <span className="text-sm text-warm-gray-400"> / {product.unit}</span>
                                                </div>
                                                <a href="/login" className="text-organic-green-500 font-bold text-sm hover:underline">Log in to buy</a>
                                            </div>
                                        </div>
                                    </div>
                                ))}

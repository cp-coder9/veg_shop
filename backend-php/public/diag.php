...
// 7. Fetch More Audit Logs
try {
    $auditLogs = \App\Services\AuditService::getLogs(['limit' => 50]);
    $results['recent_audit_logs'] = $auditLogs;
} catch (\Exception $e) {
    $results['audit_logs_error'] = $e->getMessage();
}

// 8. Count Users
try {
    $firebase = new \App\Services\FirebaseService();
    $users = $firebase->query('users', 'role', '>', '');
    $results['user_count'] = count($users);
    $results['sample_users'] = array_map(fn($u) => ['email' => $u['email'] ?? 'N/A', 'role' => $u['role'] ?? 'N/A'], array_slice($users, 0, 5));
} catch (\Exception $e) {
